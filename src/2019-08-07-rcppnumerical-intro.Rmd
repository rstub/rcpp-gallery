...
the `Numer::Func` class, whose definition is:
...
// [[Rcpp::export]]
double integral_rcpp(double a, double b)
{
    integrand f;
    double err_est;
    int err_code;
...
```{r}
b <- 10^(1:10)
Fab <- sapply(b, integral_rcpp, a = 0)
...
```{r}
tryCatch(integral_rcpp(0, 1e19), error = print)
```

Until recently it was not possible to explicitly use `Inf` as upper bound with **RcppNumerical**, but since version 0.4-0 this feature has been added.
The basic idea for handling integration with infinite bounds is to use varaible substitution to map the integral to a finite range. In the case where both limits are infinite, we use $x = (1-t)/t$, resulting in
...
respectively. These variable substitutions are implemented implicitly, so one does not have to deal with
them directly. `Numer::integrate` automatically detects whether any of the limits are
infinite, and the corresponding substitutions will be applied:
...
a functor that inherits from the `Numer::MFunc` class:
